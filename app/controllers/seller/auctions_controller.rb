class Seller::AuctionsController < ApplicationController
  ...
  def create
    ...
    if @auction.save
      ...
      SellerNotifier.perform_in((TimeDifference.between(starting_time,ending_time).in_minutes.to_i).minutes, @seller.id)
      UserNotifier.perform_in((TimeDifference.between(starting_time,ending_time).in_minutes.to_i).minutes, @auction.id)
